...
function aiseo_add_internal_links_to_post($post_id) {
    ...
    $query = $wpdb->prepare("
        SELECT p.ID, p.post_title, GROUP_CONCAT(DISTINCT t.name) AS keywords, 
               GROUP_CONCAT(DISTINCT pm.meta_value) AS additional_keywords
        FROM {$wpdb->posts} p
        ...
        WHERE p.post_status IN ('publish', 'draft') AND p.ID != %d
        GROUP BY p.ID
    ", $post_id);
    ...
    foreach ($other_posts as $other_post) {
        $keywords = explode(',', $other_post->keywords);
        $additional_keywords = explode(',', $other_post->additional_keywords);
        $used_keywords = get_post_meta($other_post->ID, '_aiseo_used_keywords', true);
        $used_keywords = $used_keywords ? explode(',', $used_keywords) : [];
        $all_keywords = array_merge($keywords, $additional_keywords, $used_keywords);
        $all_keywords = array_unique(array_filter(array_map('trim', $all_keywords)));

        foreach ($all_keywords as $keyword) {
            // Rest of the linking logic remains the same
        }
    }
...
function aiseo_add_internal_links_to_all_posts($keywords) {
    global $wpdb;
    ...
    foreach ($posts as $post) {
        ...
        foreach ($posts as $link_post) {
            if ($post->ID !== $link_post->ID) {
                foreach ($keywords as $keyword) {
                    ...
                    $replacement = '<a href="' . get_permalink($link_post->ID) . '">' . $keyword . '</a>';
                    $updated_content = preg_replace($pattern, $replacement, $updated_content, 1);
                    
                    if ($updated_content !== $post->post_content) {
                        $links_added++;
                        $post->post_content = $updated_content; // Update the reference content
                        if ($links_added >= $max_links) {
                            break 2; // Break out of both loops if max links reached
                        }
                    }
                }
            }
        ...
        if ($updated_content !== $post->post_content) {
            ...
            aiseo_log("Added {$links_added} internal links to post ID: {$post->ID}");
        }
    }
